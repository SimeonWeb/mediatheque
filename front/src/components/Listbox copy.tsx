...
const listBoxButtonVariants = cva(
	[
		...
		"[.ListboxContainer:has(.ListboxClearButton)_&]:pr-[5.5em]",
	...
	],
	{
		variants: {
			intent: {
				default: [
					"bg-primary-1 disabled:bg-primary-2 aria-readonly:bg-primary-1",
					"text-accent-1 placeholder:text-accent-1/50 disabled:text-accent-1/50 aria-readonly:text-accent-1",
					"outline-primary-4 data-focus:outline-primary disabled:outline-primary-3 aria-readonly:outline-primary-4",
				],
			},
			rounded: {
				true: "rounded-full",
...
const ListboxLabels = <TValue extends OptionValue>({
	...
	const emptyLabel = <span className="text-accent-1/50">{placeholder || "\xa0"}</span>
				...
				{selectedOptions.map(({ value, label }) => (
					<span
						...
						className={cn(
							...
							"text-accent-1 in-disabled:text-accent-1/50 in-aria-readonly:text-accent-1",
							"py-1 px-1.5",
							"rounded",
						)}
					>
	...
	return (
		<div className={cn(listBoxWrapperVariants({ size }), containerClassName)}>
			...
			>
				...
				<ListboxOptions
					...
					className={cn(
						...
						"ring ring-primary-1 shadow-[0_1rem_2rem_var(--tw-shadow-color)] shadow-primary-1",
						"transition",
						"z-40",
						menuClassName,
					)}
			...
			<Icon
				...
				size="sm"
				className="pointer-events-none col-start-1 row-start-1 mr-[1em] self-center justify-self-end"
			/>
		</div>
	)
}
